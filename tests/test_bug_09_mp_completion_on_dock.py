...
Scenario (2026-07-04 diag): the i210 firmware terminates a task at
`mowingPercentage = 99`, never 100. The FEAT-05 tracker previously
required an exact `mp = 100 ∧ state = RUNNING` to fire `run_finished`,
so a successful run whose robot returned to dock on `vs = 2` (charging)
was parked in `PAUSED_DOCKED` indefinitely and the sustained-60 s
...
Fix: completion criterion becomes `mp ≥ MP_COMPLETION_THRESHOLD (99)
...
`vs = 6` (explicit user pause) is deliberately excluded — a user
pause at `mp = 99` still holds the run so the user can resume it if
desired. `vs = 4` (mowing) and `vs = 5` (returning) also don't
qualify: the robot must actually reach the dock.
"""
...
def _open_at(
    tracker: RunTracker, mp: int, *, sub: str = "100.0", t: int = 1_000_000_000_000
) -> list:
...
def test_constants_reflect_design_decision() -> None:
    assert MP_COMPLETION_THRESHOLD == 99
...
# --------------------------------------------------------------------- #
# 2. mp=99 then dock → immediate close, label completed                 #
# --------------------------------------------------------------------- #


def test_mp_99_then_dock_arrival_closes_completed() -> None:
    """Reproduces the 2026-07-04 diag: mp peaks at 99, then the robot
    reaches dock on vs=2. The close must fire on the vs event, not
    later — no waiting for the sustained-60 s timer.
    """
    tracker = RunTracker()
    events = _open_at(tracker, mp=99)
    # Only run_started so far — mp=99 alone doesn't close.
...
def test_dock_first_then_mp_99_closes_completed() -> None:
    ...
    # A fresh type-2 with strict progress bumps mp to 99 — reopens the
...
# --------------------------------------------------------------------- #
# 4. vs=6 (user pause) is excluded — hold semantics preserved           #
# --------------------------------------------------------------------- #
...
def test_mp_98_plus_dock_does_not_complete() -> None:
    """mp = 98 is one below the threshold — must NOT complete on dock.
    Documents the exact boundary of the criterion.
    """
    tracker = RunTracker()
    _open_at(tracker, mp=98)
    events = tracker.process_vehicle_state(VS_DOCKED_CHARGING)
    assert [e for e in events if e.kind == EVENT_RUN_FINISHED] == []
    assert tracker.state == STATE_PAUSED_DOCKED


def test_mp_99_is_the_first_completing_value() -> None:
    """mp = 99 is the lowest value that triggers completion — the
    firmware peak observed on the 2026-07-04 diag run.
    """
    tracker = RunTracker()
    _open_at(tracker, mp=99)
    events = tracker.process_vehicle_state(VS_DOCKED_CHARGING)
...
# --------------------------------------------------------------------- #
# 7. mp = 100 while still mowing does NOT close                         #
# --------------------------------------------------------------------- #


def test_mp_100_while_mowing_does_not_close() -> None:
    """Design change: even mp=100 doesn't close while the robot is
    still mowing (vs=4). The completion criterion requires docked.
    This is the intended tightening from BUG-09 — a run_finished event
...
def test_mp_99_while_returning_does_not_close() -> None:
    """vs=5 (returning) is also not a completion state — the robot is
    on its way back but not yet on the dock. Wait for vs=1/2/3.
    """
    tracker = RunTracker()
    tracker.process_vehicle_state(VS_MOWING)
    _open_at(tracker, mp=99)
...
def test_sustained_timer_labels_completed_via_restore_race() -> None:
    ...
    a snapshot taken while `PAUSED_DOCKED + vs=6 + mp=99`, mutated to
    ...
    labels the close `completed` because `last_mp = 99`.
...
def test_sustained_timer_still_labels_interrupted_below_threshold() -> None:
    ...
    # vs=1 with mp < 99: fast path doesn't fire — state is PAUSED_DOCKED.
    assert tracker.state == STATE_PAUSED_DOCKED
...
def test_reset_labels_prior_run_completed_when_mp_above_threshold() -> None:
    """A fresh sub reset closes the previous run. Under BUG-09 the
    label is derived from that run's `last_mp` — so a reset after a
    prior run reached mp=99 labels the prior run `completed`, not
    `interrupted`. Handles the corner case where a new run starts
    before the tracker had a chance to fire the completion event.
    """
    tracker = RunTracker()
    _open_at(tracker, mp=99, sub="200.0", t=1_000_000_000_000)
    # Fresh sub reset (sub << ceiling) — closes the prior run and
    # opens a new one. The prior run's last_mp = 99 → completed.
    events = _process(
        tracker,
        {
            "type": 2,
