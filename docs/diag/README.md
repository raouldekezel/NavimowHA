...
| 2026-07-20 | [BUG-19](https://github.com/raouldekezel/NavimowHA/issues/114) | Post-raoul.22 (BUG-17 fix live) validation mow — does the guard suppress the vestige on the operator's actual daily rhythm? | No. Wire-shape identical to the 2026-07-19 vestige (`action = -1, mp = 100, cmp = 10000, sub = "0.0", wk = "357.63"` mid-week Monday). BUG-17 guard bypassed because the tracker was restored to `STATE_COMPLETED` from Store on HA restart (previous 2026-07-19 close persisted; `coordinator.py:183` `run_tracker.restore(tracker_snap)`), not `STATE_IDLE` — arming window dark by design (#105 fifth-edit seam map delegates post-close to #86). Packet flowed to the post-close `is_reset` branch → `_open_run(vestige)` → run anchored on vestige's `start_time`/`sub₀`/`mow_start_type`, `zones[0].cmp_max = 10000` poisoned exactly as raoul.19. Also confirmed: **no observability line was emitted** — #92's promised BUG-16/BUG-13 hook is not implemented in the deployed source. Bonus: `history[]` dump contains rows 5+10 that materialise BUG-13's zero-`sub` phantom and BUG-16's frozen-`sub` sibling in persisted form. Triaged and resolved on #114 (widen the BUG-17 guard's arming window). | [2026-07-20_bug-17_bypass-in-state-completed](2026-07-20_bug-17_bypass-in-state-completed/findings.md) |
| 2026-07-23 | [HARD-18](https://github.com/raouldekezel/NavimowHA/issues/117) | Post-deploy on `raoul.26`: does the eager start count the démarrage in a run's `duration`, and does an immediate return-to-dock commit a clean minimal aborted-start entry? | Yes to both. **Essai 1** (seeded, then interrupted after a single type-2): `duration = 78 s` spans press → first task packet, so the démarrage is counted (pre-HARD-18 this run would read ~0 s); `session_area = 0` is honest — one packet, `sub0 = sub_exit = 41.28`, a delta needs ≥2 (FEAT-06). **Essai 2** (immediate return): the arbitrated minimal entry — `result=interrupted, zones=[], session_area=None, mow_start_type=None, duration=54.2 s` real wander — `aborted_starts_committed → 1`. `invariant_deviations_observed` flat at 1 since raoul.26 as predicted. Return-transit asymmetry (#120) made concrete: essai 1's `end_time` is the last type-2, not the dock arrival. | [2026-07-23_hard-18_post-deploy-field-runs](2026-07-23_hard-18_post-deploy-field-runs/findings.md) |
| 2026-07-23 | [HARD-21](https://github.com/raouldekezel/NavimowHA/issues/123) | Are the two shims HARD-21 removes actually spent — any old-scheme unique_id left in the live registry, and has the Store re-persisted the post-HARD-20 state vocabulary on disk? | Yes, both spent. The live entity registry carries 26 navimow unique_ids, **zero** on the old FEAT-08 scheme (all `current`/`last`/`total`) → `_async_migrate_unique_ids` is dead code. The run_tracker Store re-persisted `state="idle"` after the first post-`raoul.27` mow closed → the HARD-20 `restore()` dead-vocabulary mapping has nothing left to translate. Both removed on-evidence (never on "a restart happened"); `grep "MIGRATION("` now returns zero. | [2026-07-23_hard-21_migration-retirement-evidence](2026-07-23_hard-21_migration-retirement-evidence/findings.md) |
| 2026-07-24 | [BUG-15](https://github.com/raouldekezel/NavimowHA/issues/90) | Was BUG-15 (closed 2026-07-21 "Absorbed in bug-19") actually fixed, and is the 2026-07-24 02:43 nocturnal phantom the same bug? | Not fixed. A `boundary=0 ∧ mp=0 ∧ sub=0` session-init sentinel opened a phantom run at 02:43 while the robot was docked all night — closed `interrupted`, area 0, ~10.9 h later at the raoul.28 reload. The BUG-17/BUG-19 vestige guard cannot catch it: its drop signature is the opposite shape (`mp=100 ∧ cmp=10000`). BUG-15 (task-start sentinel) and BUG-19 (task-end vestige) share a symptom, not a trigger — the absorption was a misclassification. Validated: a real sentinel always arrives ~15 s after `vs=4` (4/4 corpus), so it never IDLE-opens; only the docked spurious one does. Unrelated to HARD-19. | [2026-07-24_bug-15_recurrence-boundary-zero-sentinel](2026-07-24_bug-15_recurrence-boundary-zero-sentinel/findings.md) |
| 2026-08-23 | [FEAT-11](https://github.com/raouldekezel/NavimowHA/issues/136) | Under OS V4.3.0, what happens to battery updates while the robot is *mowing* — the same fresh-`/state`-discard freeze proven on the dock (H1), or an HTTP fallback that fires but returns a stale/pre-run value (H2)? | **H1 — same root cause as the dock.** Live mow to app-95 % + partial recharge: MQTT `/state` pushed the correct battery frequently-when-moving (wire traced a full **100→94→100** V), BUG-08 discarded every value (`held_battery=100` on all 23 pushes), and the fresh `/state` **starved HTTP** — a 17 m 41 s fetch blackout across the whole excursion. HA stayed pinned at **100** the entire time (84/84 TSV rows). HTTP is **starved, not stale** (H2 tail refuted). New detail: `/state` cadence is **battery-movement-gated** — sparse while flat (HTTP fires, correct), frequent while moving (HTTP starved) — so the only path that writes battery is available only when the battery isn't changing. Fully explains the 2026-08-21 100 %→jump→stuck report. Diagnosis only; no fix. | [2026-08-23_feat-11_v43-battery-mowing](2026-08-23_feat-11_v43-battery-mowing/findings.md) |
...
- Two file flavours per action: `.mqtt.log` for the integration's own
  log lines, `.sensors.tsv` for periodic polls of Navimow entities.

## Capturing the `.mqtt.log`

The `.mqtt.log` is a slice of `docker logs hass` **filtered to this
integration's own loggers** (`custom_components.navimow` and
`mower_sdk`) — never a raw, unfiltered dump. A bare `docker logs hass`
slice also captures every other component active in the same window
(presence / `device_tracker`, reverse-geocoding, camera proxies,
weather, automations): analysis noise that **also carries unrelated
third-party PII** — household-member locations, camera access tokens,
LAN addresses. Logger scoping is the first line of PII defense; the
value-level redaction below is the second. **This is mandatory**, not
best-effort.
...
docker logs hass --since <ISO-ts> 2>&1 \
  | sed -r 's/\x1b\[[0-9;]*m//g' \
  | awk '/ (DEBUG|INFO|WARNING|ERROR|CRITICAL) \(/ { keep = /\[custom_components\.navimow|\[mower_sdk/ } keep' \
  > NN_<action>.mqtt.log
```

The `awk` starts a new record on each `… LEVEL (thread) …` line and keeps
it — plus its continuation lines — only when the logger is `navimow` or
`mower_sdk`. Format-agnostic (works with or without `docker logs -t`).

**Caveat:** an unretrieved async-task crash is logged by HA core under
`[homeassistant]`, with `navimow` only in the stack frames — a
