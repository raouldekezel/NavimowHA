...
| 2026-07-04 | [BUG-09](https://github.com/raouldekezel/NavimowHA/issues/51) | On the first live raoul.8 run, why is the tracker parked in `paused_docked` ~50 min after the robot returned to dock, with `sensor.<slug>_last_run_result` still `unknown`? | `mowingPercentage` peaked at **99** (never 100) and the robot returned directly to charging (`vs=2`) — the state machine's three exit paths are all unreachable in this shape. Path 1 (`mp=100 → COMPLETED`) never fires; path 2 needs a fresh reset; path 3 (`vs ∈ {1, 3}` sustained 60 s) is design-blocked by `vs=2` (recharge-pause hold). Battery climbed monotonically from 58 % to 82 % across the observation, ruling out a mid-run recharge — this is a terminal dock the tracker can't recognise. Fix design open (`mp` completion ceiling + zone `cmp=10000` fast path, or `mowingWeekArea` stagnation backstop). | [2026-07-04_bug-09_paused-docked-mp-99](2026-07-04_bug-09_paused-docked-mp-99/findings.md) |
